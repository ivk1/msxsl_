...
_strndup(const char * psSrc, int cchSrc)
{
    ...
    pszDest = (char *) __malloc(cchSrc * sizeof(char) + 1);
    if (pszDest)
    {
...
_wcsndup(const WCHAR * pwsSrc, int cwchSrc)
{
    ...
    pwszDest = (WCHAR *) __malloc(cwchSrc * sizeof(WCHAR) + 2);
    if (pwszDest)
    {
...
*/
WCHAR * __cdecl
__itow(int iVal, WCHAR * pwszBuf, int iRadix)
{
    if (iRadix == 10 && iVal < 0)
...
*/
WCHAR * __cdecl
__wcscpy(WCHAR * pwszDest, const WCHAR * pwszSrc)
{
    WCHAR * pwsz = pwszDest;
...
*/
int __cdecl
__wcsnicmp(const WCHAR * pwsz1, const WCHAR * pwsz2, size_t cwch)
{
    WCHAR wch1, wch2;
    int iRes = 0;
...
_assert(const char * psz, const char * pszFile, unsigned iLine)
{
    ...
    WriteAsMBCS(g_hStdErr, __itow((int) iLine, rgwchLine, 10));
    WriteAsMBCS(g_hStdErr, "\r\n   Text: ");
