...
MSXMLCommandLine::ParsedToken(const WCHAR * pws, const WCHAR ** ppwsEnd, TokenType type)
{
    ...
    case TokSwitch:
        ...
        case 2:
            if (!__wcsnicmp(pws, L"xe", 2))
            {
                _fResolveExt = false;
                break;
            }
            else if (!__wcsnicmp(pws, L"xw", 2))
            {
                _fStripWS = true;
                break;
            }
            else if (!__wcsnicmp(pws, L"pi", 2))
            {
                switch (_stateOption)
                {
                case StateStylesheet:
    ...
    case TokQuotedOption:
        ...
        case StateVersion:
            // Version may be 2.6, 3.0, or 6.0
            //if (!memcmp(pws, L"4.0", 3 * sizeof(WCHAR)))
            //{
            //    _version = MSXMLInfo::VERSION_40;
            //}
			if (!memcmp(pws, L"6.0", 3 * sizeof(WCHAR)))
			{
				_version = MSXMLInfo::VERSION_60;
			}
