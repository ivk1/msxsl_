...
Cleanup:
    __free((void *) pwszHeader);
    __free((void *) pwszUsage);
}
...
FormatTiming(DWORD dwTiming, WCHAR * pwszTiming)
{
    ...
    __itow(dwTiming, pwszTiming + cwch, 10);
...
PrintTimings(MSXMLInfo * pMSXMLInfo, TransformHelper * transHelper)
{
    ...
    ChkSucc(Resources::FormatResourceMessage(
		        ...
		        FormatTiming(transHelper->GetSourceTime() + transHelper->GetStylesheetTime() + transHelper->GetCompileTime() + transHelper->GetExecutionTime(), rgwchTotalTime),
                NULL));
...
Cleanup:
    ...
    if (FAILED(hr))
    {
        ...
        if (SUCCEEDED(Resources::FormatResourceMessage(MSXSL_E_CMDLINE_ERROR, &pwszText, Exception::GetContext(), __itow(hr, rgwchInt, 16), Exception::GetText(), NULL)))
        {
            WriteAsMBCS(g_hStdErr, L"\r\n");
