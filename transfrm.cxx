...
TransformHelper::GetStyleNameFromPI(IXMLDOMDocument * pInput)
{
    ...
    ChkSuccCOM(pInput->selectNodes((BSTR)L"processing-instruction('xml-stylesheet')", &pNodeList));
    ...
    if (pNode)
    {
        ...
        while (true)
        {
            ...
            // Look for 'href' tag
            isHref = !__wcsnicmp(pwszBegin, L"href", 4) && (pwszBegin[4] == L'=' || ParseCommandLine::IsWhitespaceChar(pwszBegin[4]));
    ...
    if (fSuccess == VARIANT_FALSE)
    {
        ...
        if (ulLine != 0)
        {
            ...
            Exception::CreateFromResource(MSXSL_E_PARSE_ERROR,
                                          ...
                                          __itow((int) ulLine, rgwchLine, 10),
                                          __itow((int) ulColumn, rgwchColumn, 10),
                                          bstrReason);
        }
        else
        {
