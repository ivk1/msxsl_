...
ParameterMgr::AddParameter(const WCHAR * pwsName, int cwchName, int cwchPrefix)
{
    ...
    if (_cParams >= _cMaxParams)
    {
        ...
        pNewParams = (Param *) __malloc(_cMaxParams * sizeof(Param));
        ChkMem(pNewParams);
...
NamespaceMgr::AddPrefix(const WCHAR * pwsPrefix, int cwchPrefix)
{
    ...
    if (_cNSDecl >= _cMaxNSDecl)
    {
        ...
        pNewNSDecl = (NSDecl *) __malloc(_cMaxNSDecl * sizeof(NSDecl));
        ChkMem(pNewNSDecl);
...
StringConvert::ConvertToMBCS(const WCHAR * pwsz, int cwch)
{
    ...
    else
    {
        ...
        {
            __free(_pbDynamic);
            _pbDynamic = (BYTE *) __malloc(iLen + 1);
            ChkMem(_pbDynamic);
            _iDynamicLen = iLen;
        }
...
Resources::FormatMessage(ID resid, const WCHAR ** ppwszOut, DWORD dwFlags, const WCHAR * pwszArg, va_list * pva)
{
	...
	fIsNT = true; // who is using win95/98 today?
    ...
    if (pwszArg)
    {
        ...
        ppvArgs = (void **) __malloc(cArgs * sizeof(void *));
        ChkMem(ppvArgs);
...
Cleanup:
    if (!dwRet && resid != MSXSL_E_SYSTEM_ERROR)
    {
        ...
        WCHAR rgwchID[16];
        hr = FormatResourceMessage(MSXSL_E_SYSTEM_ERROR, ppwszOut, __itow(resid, rgwchID, 16), NULL);
    }
...
Exception::Clear()
{
    __free((void *) s_pwszText);
    s_pwszText = NULL;
    __free((void *) s_pwszContext);
    s_pwszContext = NULL;
}
